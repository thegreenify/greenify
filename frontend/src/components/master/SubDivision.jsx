...
const SubDivision = ({ state }) => {
    ...
    await axios
      ...
      .then(async (res) => {
        ...
        const division = res.data.cities.filter(
          ({ divisionName }, index) =>
            !zon.includes(divisionName, index + 1) && divisionName != null
        );
  ...
  const data = state.filter(
    (item) => item.subdivisionName != null || item.subdivisionName != undefined
  );
  ...
  return (
    <div style={{ paddingBottom: 50 }}>
      ...
      >
        <p className="summary">Subdivision Summary</p>
        <div style={{ display: "flex" }}>
          <div className="total">
            <h2 style={{ padding: "20px 0" }}>{state.length}</h2>
            <p style={{ padding: "20px 0" }}>Total Subdivision</p>
          </div>
          <div className="total">
            <h2 style={{ padding: "20px 0" }}>1</h2>
            <p style={{ padding: "20px 0" }}>Active Subdivision</p>
          </div>
          <div className="total">
            <h2 style={{ padding: "20px 0" }}>1</h2>
            <p style={{ padding: "20px 0" }}>Inactive Subdivision</p>
          </div>
        </div>
      </div>
      <div
          ...
          <div className="modal-content">
            ...
            >
              ...
              <div className="input-style" style={{ width: 250 }}>
                ...
                <input
                  onChange={(e) =>
                    setArea({ ...area, subdivisionName: e.target.value })
                  }
                />
              </div>
            </div>
